...
def generate_padding(size):
    return random.choices(range(0x00, 0xff), k=size)
def init_config():
    global config
...
class EnhancedFakeTLSStreamReader(asyncio.StreamReader):
    async def read(self, n=-1):
        data = await super().read(n)
        if len(data) >= 5:
            obfuscation_layer = generate_padding(5)
            data = bytes(obfuscation_layer) + data
        return data

class EnhancedFakeTLSStreamWriter(asyncio.StreamWriter):
    def write(self, data):
        obfuscation_layer = generate_padding(5)
        super().write(bytes(obfuscation_layer) + data)
...
class CryptoHelper:
    ...
    def create_cipher(self, iv):
        return Cipher(algorithms.AES(self.key), modes.CTR(iv), backend=self.backend)

    def encrypt(self, plaintext, iv):
        encryptor = self.create_cipher(iv).encryptor()
        return encryptor.update(plaintext) + encryptor.finalize()

    def decrypt(self, ciphertext, iv):
        decryptor = self.create_cipher(iv).decryptor()
...
async def handle_fake_tls_handshake(handshake, reader, writer, peer):
    ...
    for user in config.USERS:
        ...
        crypto = CryptoHelper(PROXY_SECRET)
        encrypted_handshake = crypto.encrypt(handshake, iv)
        ...
        reader = EnhancedFakeTLSStreamReader(reader)
        writer = EnhancedFakeTLSStreamWriter(writer)
...
async def start_server():
    server = await asyncio.start_server(
        handle_fake_tls_handshake, "0.0.0.0", 443
    )
